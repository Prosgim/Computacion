...
def voraz_x_coste(costMatrix):
    ...
    for i in range(costMatrix.shape[0]):
        min = costMatrix[i][0]
        for j in range(costMatrix.shape[1]):
            if (costMatrix[i][j] < min and j not in visitedColumns):
                min = costMatrix[i][j]
                minRow = [i, j]
                minColumn = j        
        costVector.append([min, minRow[0], minRow[1]])  
        visitedColumns.append(minColumn)       
             
    score = 0
    solution = []            
    for row in costVector:
        score += row[0]
        solution.append(row[2])    
    return score,solution

def voraz_combina(costMatrix):
    score1, sol1 = voraz_x_pieza(costMatrix)
    score2, sol2 = voraz_x_instante(costMatrix)
    score3, sol3 = voraz_x_coste(costMatrix)
