...
;TEMPLATES
    ...
    (deftemplate Secado
        ...
        (slot WARNING(type SYMBOL) (default OFF))) ;Warning (OFF por defecto)
...
;RULES WARNING
    (defrule warning
    (velocidad very alta)
    ?S <- (Secado (temq ?temperatura) (peso ?peso) (velocidad ?velocidad) (modo ?modo) (ECO ?ECO) (warning ?w))
    (test (eq ?w  OFF))
    => 
    (modify ?v (warning ON))
    (printout t "Peligro! La ropa podría quedar demasiado arrugada"crlf)
    )
